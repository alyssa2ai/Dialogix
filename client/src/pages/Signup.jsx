...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/auth/signup', { username: form.username, email: form.email, password: form.password });
      ...
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8" style={{ border: '0.5px solid rgba(168,85,247,0.2)' }}>
            ...
            {['username','email','password','confirmPassword'].map(field => (
              <div key={field}>
                ...
                <input
                  ...
                  required
                  placeholder={field === 'email' ? 'you@cosmos.io' : field === 'username' ? 'callsign' : '••••••••'}
                  style={{
          ...
          <p style={{ textAlign: 'center', color: 'var(--text-dim)', fontSize: '13px', marginTop: '24px' }}>
            Already connected?{' '}
            <Link to="/login" style={{ color: '#a855f7' }}>Reestablish link</Link>
          </p>
        </div>
